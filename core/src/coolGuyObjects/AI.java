...
public class AI extends GameObject{
	...
	public void friendCollision(AI r){
		if(count > 1){
			if(overlapsTarget && this.getHitBox().getX() != r.getHitBox().getX()){
				if(this.getRight().overlaps(r.getLeft())){
					r.action(1, r.full.x, full.y+full.height);
					r.action(2, r.full.x, full.y);
					if(this.getTop().overlaps(r.getBottom())){
						velocity = 0;
					}
				}
			}else{
